...
<section class="breadcrumb_area">
            ...
            <div class="container">
                <div class="breadcrumb_content text-center">
                    <h1 class="f_p f_700 f_size_50 w_color l_height50 mb_20">Forget Password</h1>
                    <p class="f_400 w_color f_size_16 l_height26">Enter your administrator email address to receive an email with a link to reset your password.</p>
                </div>
            </div>
        </section>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-7">
                            <div class="login_info">
                                <h2 class="f_p f_600 f_size_24 t_color3 mb_40">Recover your password</h2>
                                <form action="<?php echo base_url('do-forgot-password')?>" class="login-form sign-in-form" id="do-forgot-password-form">
                                    ...
                                    <div class="form-group text_box">
                                        ...
                                        <input type="text" name="email" placeholder="saasland@gmail.com">
                                        <span class="text-danger"></span>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <button type="submit" class="btn_three">Recover Password</button>
                                    </div>
                                </form>
                            </div>
                        </div>
...
<?php $this->load->view('partial/footer_script'); ?>
    ...
    $(document).on('submit', '#do-forgot-password-form', function (event) {
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.status === 200) {
                    if (resp.message) {
                        $('#do-forgot-password-form').find('.alert-success').html(resp.message).show();
                        $('#do-forgot-password-form').trigger("reset");
                    }
                } else {
                    $.each(resp.message, function (key, val) {
                        $('#do-forgot-password-form').find('[name="' + key + '"]').closest('.form-group').find('.text-danger').html(val);
                    });
                }
            }
        }).fail(function () {
