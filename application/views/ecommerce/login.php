    ...
    if($timeOfDay == 'am'){
        ...
        echo  'Good Afternoon';
              ...
              <div class="form-style">
                  <form action="<?php echo base_url($domain_name.'/do-login-ecomm').'.html';?>" method="post" id="do-login">
                    ...
                    <div class="row align-items-center">
                      <div class="col-sm-6">
                        <button type="submit" class="signup_btn">SignIn</button>
                      </div>
<!--                      <div class="col-sm-6 text-right">
            ...
            <div class="login-link">
              Don't have an account? <strong><a href="<?php echo base_url($domain_name.'/ecomm-signup').'.html'?>">SignUp</a></strong> or <strong><a href="<?php echo base_url($domain_name.'/index').'.html'?>">Back to Home</a></strong>
            </div>
        </div>
        <div class="col-md-5 d-none d-md-block form-right-part">
            ...
            <div class="middle-part">
              ...
              <div class="haed_text"><?php echo $this->all_function->get_cms_by_page_section_type($org_id,'Login-Page', 'Login-Page-right-side-title'); ?></div>
              <div class="para">
                <?php echo $this->all_function->get_cms_by_page_section_type($org_id,'Login-Page', 'Login-Page-right-side-description'); ?>
              </div>
            </div>
            <div class="address">
              <i class="fa fa-map-marker" aria-hidden="true"></i>
              <div class="address_text">
                <?php echo $this->all_function->get_cms_by_page_section_type($org_id,'Login-Page', 'Login-Page-right-side-address-text'); ?>
...
function doSomething() {
    ...
    $(document).on('submit', '#do-login', function (event) {
            ...
            success: function (resp) {
                if (resp.status === 200) {
                    ...
                    $.each(resp.message, function (key, val) {
                        $('#do-login').find('[name="' + key + '"]').closest('.form-group').find('.text-success').html(val);
                    });
                    $("#do-login").attr('action','<?php echo base_url($domain_name.'/ecomm-login-verify-otp').'.html'?>');
                } else if(resp.status === 300){
                //location.reload();
                window.location = '<?php echo base_url($domain_name.'/update-company-information').'.html';?>';
                ...
                window.location = '<?php echo base_url($domain_name.'/index.html');?>';
                    ...
                    $.each(resp.message, function (key, val) {
                        $('#do-login').find('[name="' + key + '"]').closest('.form-group').find('.text-danger').html(val);
                    });
                }
            }
        }).fail(function () {
